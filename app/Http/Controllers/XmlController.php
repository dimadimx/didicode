...
EOF;
    ...
    {
        return view('xml.index', ['betAmounts' => $this->parseBetAmount(self::$exampleXml)]);
    }
     ...
     * @return array
     */
    private function parseBetAmount(string $xmlData): array {
        $betAmounts = [];
        try {
            $xml = new \SimpleXMLElement($xmlData);
            if (($SimpleXMLElements = $xml->xpath('*/BetInfo/BetAmount')) !== false) {
                // reset array to single
                $betAmounts = array_map('current', $SimpleXMLElements);
            }
        } catch(\Exception $e) {
            abort(404);
        }
